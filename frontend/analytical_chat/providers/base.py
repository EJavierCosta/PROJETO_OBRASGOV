...
def build_sql_prompt(request: SQLGenerationRequest) -> str:
    ...
    return (
        ...
        "relações filhas.\n"
        "Para perguntas sobre contrato mais caro/maior valor ou contrato mais barato/menor "
        "valor por município, use sempre um SELECT completo após as CTEs: locations AS "
        "(SELECT DISTINCT project_id FROM gold.vw_project_location_current WHERE "
        "municipality_name ILIKE '<município>'), candidate_contracts AS (SELECT c.project_id, "
        "c.contract_source_id, c.contract_number, c.supplier_name, c.valor_global_contrato "
        "FROM gold.vw_project_contract_current c INNER JOIN locations l ON c.project_id = "
        "l.project_id), ranked AS (SELECT cc.project_id, m.project_name, "
        "cc.contract_source_id, cc.contract_number, cc.supplier_name, "
        "cc.valor_global_contrato FROM candidate_contracts cc INNER JOIN "
        "gold.vw_market_overview_current m ON cc.project_id = m.project_id ORDER BY "
        "cc.valor_global_contrato DESC NULLS LAST LIMIT 1) SELECT project_id, "
        "project_name, contract_source_id, contract_number, supplier_name, "
        "valor_global_contrato FROM ranked. Use ASC NULLS LAST LIMIT 1 para menor valor e "
        "DESC NULLS LAST LIMIT 1 para maior valor. Nunca deixe um alias de CTE seguido de "
        "JOIN sem o SELECT ... FROM correspondente.\n"
