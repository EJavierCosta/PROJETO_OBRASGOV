...
| AC-006 | `pytest` frontend e abertura do Streamlit no container | Passou: 7 testes frontend, endpoint de saúde `200 ok` e consultas Gold reais validadas | `tests/frontend/test_smoke.py`; `http://127.0.0.1:8501/_stcore/health` |
| AC-007 | `pytest`; Ruff; compileall; dbt build | Passou: 17 testes, lint e compilação limpos, dbt 153/153 | comandos executados em 22/08/2026 |
...
| AC-010 | `test_source_update_change_marks_run_failed` | Passou | `pytest`: 17 passed |
...
| AC-013 | Renderização no navegador local contra `http://localhost:8501/` | Passou: logo, filtros, snapshot, KPIs, mapa, situação e tabela visíveis | inspeção visual de 22/08/2026 |
